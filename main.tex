...
From the late 60s computer programs have been used to compute and design the orbit and optics of particle beams in CERN sychrotrons and related transfer and experimental lines. In the 80s, as part of the LEP collider design, the MAD program was develope and later evolved into MAD-X with emerged as the de-facto standard in optics calculations and operation of accelerators at CERN during the LHC era.
...
Many more tools have also been developed over the years for the study of specific phenomena. For example HEADTAIL and its evolution PyHEADTAIL have been the workhorse simulate transverse collective instabilities.
...
Two particles species of great interest for hadron therapy are Carbon and Helium. It turns out that the properties of such species align just right to allow them to propagate at the same time in the same ring. Studies by a collaboration at MedAustron in Austria are working on using beams made of a mix of the two species for ``online range verification'' during the patient irradiation. In practice, while the accelerated carbon ions are deposited in the are to be treated, for example a tumor, the helium ions go through the patient and can be used for precise localization of the beam. The research team is using Xsuite to simulated and optimize this novel therapy technique.
...
% \begin{itemize}
%     \item Modelling of novel accelerators represents it's own challenges
...
Following the great success of the integreated LEP and LHC program, the next highest energy collider studies are looking towards lepton colliders prior to larger hadron machines, such as the FCC study at CERN, or CEPC study at IHEP.

The current state-of-the-art lepton collider is SuperKEKB, located at KEK, Tsukuba Japan.
...
As SuperKEKB strives to realise their ambitious target parameters, simulation developments have been key.
A fabulously challenging machine, the effort to describe SuperKEKB has lead to not only developments of the Xsuite code, but further developments of the collaborative links between CERN and our colleagues at KEK.
Supported strongly by international schemes, such as the European Union’s Europe America Japan Accelerator Development Exchange scheme, we are benefitting greatly from the increase in knowledge sharing between our organisations.

On the technical front, SuperKEKB represents a key challenge due to the overlap of it’s experimental solenoid (used by the detectors to enable the identification of particles) and the final focus quadrupoles (used to squeeze the beams down to heights on the nano-metre scale)!
The implementation of this feature has opened new avenues for the study of future machines like the FCC also.
...
One of Xsuite’s defining features is its modular architecture, which allows different simulation modules to be seamlessly integrated. This modularity enables the study of complex scenarios where multiple beam dynamics effects must be considered simultaneously—a capability that is becoming increasingly essential in modern accelerator design.

The demand for such combined-effects simulations was first driven by the challenging requirements of the Future Circular electron–positron Collider (FCC-ee). In this context, a wide range of phenomena must be accounted for: acceleration in RF cavities, synchrotron radiation emission, magnet strength tapering to follow local beam energy, beam–beam interactions, scattering with collimators, and more. Xsuite provides the framework to simulate these effects concurrently, in a self-consistent and flexible way.

What makes this capability even more remarkable is its application to a machine as ambitious as FCC-ee: a 91 km-circumference collider featuring over 80,000 beamline elements. The ability to simulate such a large and complex system—including both collective and single-particle effects—is a strong demonstration of Xsuite’s power.

As a concrete example, integrated simulations combining beam–beam effects, collimation, synchrotron radiation emission, tapering, and acceleration have been performed using Xsuite. These simulations yield detailed predictions such as the distribution of beam losses around the FCC-ee ring (see figure). Such studies are crucial for validating that the machine can operate safely while delivering its target luminosity to the experiments. They provide a deeper understanding of how various physical effects interact and help guide key design decisions early in the project lifecycle.
...
By now Xsuite has organically become the production tool for over a hundred accelerator physicists: many studies that would previously take weeks, or even years, to prepare and execute, can now be conducted in a matter of hours. In view towards High Luminosity LHC work is underway to introduce Xsuite also to operation, where first tests have been promising. In 2024, for the first time the first full proof-of-concept LHC cycle was designed entirely with Xsuite, and tested in a Machine Development session. While relying on a modern and supported tool has clear advantages, there are also less tangible ones: e.g. the opportunities it could create for synergy and mobility between the teams that design future machines and those that operate the current ones. In the meantime, if Xsuite is relevant for your work, go ahead and give it a try: the Xsuite team is eager to hear your feedback.
